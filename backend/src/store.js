...
import { getDb, initDb } from './activityDb.js';

/**
 * Unified persistence for everything the HRT portals write: call tracking,
 * follow-ups, patient overrides (edits / delivery / abortion status), the
 * activity call log and the status-update approval queue.
 *
 * Two backends:
 ...
 *    Calls/follow-ups/overrides are mirrored in memory so the many read
 *    paths stay synchronous.
 *  - no DATABASE_URL   → the existing local files (JSON + SQLite), unchanged.
 *
...
const mem = { calls: {}, followups: {}, overrides: {} };
let pool = null;
let overridesVer = 0;
...
export function getCallsMap() {
  return usingPostgres ? mem.calls : loadJson(CALLS_FILE);
}

export function getFollowupsMap() {
  return usingPostgres ? mem.followups : loadJson(FOLLOWUP_FILE);
}

export function getOverridesMap() {
  return usingPostgres ? mem.overrides : loadJson(OVERRIDES_FILE);
}
...
export async function addCall(key, entry) {
  ...
  calls[key].push(entry);
  saveJson(CALLS_FILE, calls);
  return calls[key].length;
}
...
export async function addFollowup(key, entry) {
  ...
  if (!fu[key]) fu[key] = [];
  fu[key].push(entry);
  saveJson(FOLLOWUP_FILE, fu);
  return fu[key].length;
}
...
export async function removeOverride(...keys) {
  ...
  if (usingPostgres) {
    ...
    const ov = loadJson(OVERRIDES_FILE);
    for (const key of keys) {
      if (key in ov) { delete ov[key]; removed = true; }
    }
    if (removed) saveJson(OVERRIDES_FILE, ov);
  }
...
/** One-time import of any local legacy stores into empty Postgres tables. */
async function migrateLegacy() {
  const count = async (t) => (await pool.query(`SELECT COUNT(*)::int AS c FROM ${t}`)).rows[0].c;

  if (fs.existsSync(CALLS_FILE) && (await count('mother_calls')) === 0) {
    const legacy = loadJson(CALLS_FILE);
    let n = 0;
    for (const [uid, entries] of Object.entries(legacy)) {
      for (const e of entries || []) {
        await pool.query('INSERT INTO mother_calls (mother_key, entry) VALUES ($1, $2)', [uidToKey(uid), e]);
        n += 1;
      }
    }
    if (n) console.log(`[STORE] Migrated ${n} call entries from call_tracking.json`);
  }
  if (fs.existsSync(FOLLOWUP_FILE) && (await count('mother_followups')) === 0) {
    const legacy = loadJson(FOLLOWUP_FILE);
    let n = 0;
    for (const [uid, entries] of Object.entries(legacy)) {
      for (const e of entries || []) {
    ...
    if (n) console.log(`[STORE] Migrated ${n} follow-up entries from followup_data.json`);
  }
  if (fs.existsSync(OVERRIDES_FILE) && (await count('patient_overrides')) === 0) {
    const legacy = loadJson(OVERRIDES_FILE);
    let n = 0;
    for (const [uid, data] of Object.entries(legacy)) {
      await pool.query(
    ...
    if (n) console.log(`[STORE] Migrated ${n} patient overrides from patient_overrides.json`);
  }
  if (fs.existsSync(DB_PATH) && (await count('call_logs')) === 0) {
    try {
      initDb();
...
export async function initStore() {
  if (!usingPostgres) {
    initDb();
    console.log('[STORE] Local mode — JSON files + SQLite (set DATABASE_URL for PostgreSQL)');
    return;
  }
  ...
  console.log(`[STORE] PostgreSQL ready — ${nCalls} calls, ${nFu} follow-ups, ${Object.keys(mem.overrides).length} overrides loaded`);
}
